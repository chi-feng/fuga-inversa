...
type Buffers = Partial<Record<Part, AudioBuffer>>;
type Pending = { controller: AbortController; promise: Promise<AudioBuffer> };
type Retiring = { sources: AudioBufferSourceNode[] };
...
function loopExcerpt(context: AudioContext, buffer: AudioBuffer, start: number, end: number) {
...
export class ScorePlayer extends EventTarget {
  ...
  buffers = new Map<Instrument, Buffers>();
  pending = new Map<string, Pending>();
  recordedDurations = new Map<Instrument, number>();
  ...
  loadBuffer: (context: AudioContext, url: string, signal?: AbortSignal) => Promise<AudioBuffer>;
  assetBase: string;

  constructor({ timings, assetBase = './assets/audio/', createContext = createPlaybackContext,
    loadBuffer = async (context: AudioContext, url: string, signal?: AbortSignal) => {
  ...
  get decodedBytes() {
    const held = new Set<AudioBuffer>();
  ...
  pruneBuffers() {
    ...
    for (const [instrument, buffers] of this.buffers) {
      if (instrument !== this.instrument) this.buffers.delete(instrument);
        ...
        for (const part of Object.keys(buffers) as Part[]) if (!wanted.includes(part)) delete buffers[part];
  ...
  async loadPart(instrument: Instrument, part: Part) {
