...
import { initializeNativePlayback } from './native-playback.ts';
...
const nativePlayback = initializeNativePlayback({
  audio: fallback, buttons: $$<HTMLButtonElement>('[data-instrument]'),
...
async function enhanceEdition() {
  ...
  function update() {
    ...
    for (const button of $$('[data-context]')) {
      const expected: Voice[] = button.dataset.context === 'upper' ? ['soprano', 'alto'] : VOICES;
  ...
  for (const button of $$('[data-instrument]')) button.addEventListener('click', () => {
    contextRequest++;
    player.setInstrument(button.dataset.instrument as Instrument).catch(report);
  });
  ...
  for (const button of $$('[data-context]')) button.addEventListener('click', async () => {
    ...
    try {
      ...
      if (request === contextRequest) report(error as Error);
    }
  });
